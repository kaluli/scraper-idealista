...
async function deleteJuanCarlos() {
  try {
    console.log('🗑️  Eliminando todos los pisos de Juan Carlos I...\n');

    // Buscar todos los pisos de Juan Carlos I
    const listingsToDelete = await prisma.listing.findMany({
      where: {
        neighborhood: {
          contains: 'Juan Carlos I',
        },
      },
      select: {
        id: true,
    ...
    if (listingsToDelete.length === 0) {
      console.log('⚠️  No hay pisos de Juan Carlos I para eliminar.');
      return;
    }
    ...
    if (remaining > 0) {
      console.log(`\n⚠️  Aún quedan ${remaining} pisos de Juan Carlos I.`);
    } else {
      console.log(`\n✅ Todos los pisos de Juan Carlos I han sido eliminados.`);
    }
